...
# Origin date for offsets
origin <- as.Date("2020-01-01")

# Define new edge‐case scenarios for missing tests
edge_cases_missing <- list(
    # 13: only one A positive, two B positives
    list(id = 13, A = c(0),      B = c(0, 90)),
    # 14: two A positives, only one B positive
    list(id = 14, A = c(0, 90),  B = c(0)),
    # 15: no A positives, two B positives
    list(id = 15, A = c(),       B = c(0, 90)),
    # 16: two A positives, no B positives
    list(id = 16, A = c(0, 90),  B = c())
)

# Build test_A_pos_missing and test_B_pos_missing
test_A_pos <- map_dfr(edge_cases_missing, ~ tibble(
    person_id = .x$id,
    date      = origin + .x$A,
    result    = "Positive"
))

test_B_pos <- map_dfr(edge_cases_missing, ~ tibble(
    person_id = .x$id,
    date      = origin + .x$B,
    result    = "Positive"
))
...
# Inspect
print("Test A positives (missing‐test edge cases):")
print(test_A_pos)

print("Test B positives (missing‐test edge cases):")
print(test_B_pos)
